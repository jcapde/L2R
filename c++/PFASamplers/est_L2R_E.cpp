...
class L2RInferenceV1 : public SamplingInference {
public:
  L2RInferenceV1() {
    name = "L2R_E";
    register_method();
  ...
  virtual std::unique_ptr<PFAInferenceMethod> make_new() {
    std::unique_ptr<PFAInferenceMethod> p(new L2RInferenceV1());
    return p;
  }
  ...
  void log_p_y_word_given_previous_words_sampling(int w, int num_non_zeros, InEVector y, InEMatrix Phi, 
    ...
    int num_partitions = binom(y(w) + num_factors - 1, num_factors - 1);
    //std::cout << "Num partitions: " << num_partitions << std::endl;
    EVector log_p_y_partition(num_partitions);
    ...
    for (int s = 0 ; s < _num_samples ; s++) {
        ...
        if (w < num_non_zeros) {
          ...
          VectorXdVisitor visitor = [&log_p_y_partition, &nbin_r, &nbin_p, &num_factors, &current_partition, &w]
            (const InEVector partition)->bool {
              //std::cout << "part:" << partition<< std::endl;
              for (int k = 0; k < num_factors ; ++k) {
                log_p_y_partition(current_partition) += log_neg_bin_pdf(partition(k), nbin_r(k), nbin_p(k, w));
              }
              current_partition++;
              return false;
          };
          ...
          log_p_y_partition.setZero();
          for_each_combination_VectorXd(y(w), num_factors, visitor);
          //std::cout << "Computed "<< current_partition <<" combinations" << std::endl;
          log_p_y_w(s) = log_sum_exp_vec(log_p_y_partition);
...
//     
  ...
  virtual ~L2RInferenceV1() {};
private:
  std::unique_ptr<EMatrix> _xips_ptr;
};

static L2RInferenceV1 l2r_inference_v1_exemplar;
